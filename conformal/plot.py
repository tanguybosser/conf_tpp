...
from conformal.dataframes import make_df_mean


def get_conformalizers_order_joint():
    #return ['Naive', 'HDR', 'C-CQRAll', 'C-Naive', 'C-HDR']
    #return ['Naive', 'Naive-RAPS', 'HDR', 'C-Naive', 'C-Naive-RAPS', 'C-HDR', 'C-ConstAPS']
    #return ['C-ConstAPS', 'C-ConstAPSCond']
    #return ['Naive', 'HDR', 'C-CQRAll', 'C-Naive','C-Naive-RAPS', 'C-HDR']
    return ['N-QRL-RAPS', 'N-HDR-RAPS', 'N-HDR', 'C-QRL-RAPS', 'C-HDR-RAPS', 'C-HDR']

def get_conformalizers_order_time():
    return ['N-QR', 'N-QRL', 'N-HDR-T', 'C-Const', 'C-QR', 'C-QRL', 'C-HDR-T']

def get_conformalizers_order_mark():
    return ['N-APS2', 'N-RAPS', 'C-PROB2', 'C-APS2', 'C-RAPS']
...
def plot_scores(scores, q, alpha):
    ...
    ax.axvline(q.detach().cpu(), ymin=0, ymax=1 - alpha, label=r'$\hat{q}$', color='r', linestyle='--')
    ax.axhline(1 - alpha, xmin=0, xmax=q.detach().cpu().item(), label=r'$1 - \alpha$', color='r', linestyle='--')
    ax.legend()
    return fig
...
def plot_joint_densities(self, preds, past_events, target_time, target_label):
    ...
    nb_steps = 500
    alpha = torch.linspace(0.01, 0.7, nb_steps, device=self.args.device)[None, :].expand(batch_shape + (nb_steps,))
...
def plot_metrics_on_real_world_datasets(df, alpha, x='dataset', type='joint'):
    ...
    if type in ['time', 'joint']:
        metrics = ['coverage', 'relative_length', 'gmean_length', 'wsc', 'cond_coverage_error_z']
    else:
        metrics = ['coverage', 'length', 'wsc', 'cond_coverage_error_z']
    ...
    for i, metric in enumerate(metrics):
        axis = axes[i]
        g = sns.barplot(data=df, x=x, y=metric, hue='conformalizer', hue_order=conformalizer_order, palette=color_palette, ax=axis)
        ...
        if metric in ['length', 'relative_length', 'cond_coverage_error_z', 'cond_coverage_error_repr'] and type != 'mark':
...
def plot_metrics_on_real_world_datasets_with_separate_axes(df, alpha, x='dataset', type='joint'):
    ...
    for i, metric in enumerate(metrics):
        for j, x_value in enumerate(x_values):
            ...
            g = sns.barplot(data=df_plot,  x=x, y=metric, hue='conformalizer', hue_order=conformalizer_order, palette=color_palette, ax=axis)
            ...
            if metric in ['coverage', 'wsc']:
                axis.set_ylim(0, 1)
                axis.axhline(y=1 - alpha, color='black', linestyle='--')
            # if metric in ['length', 'cond_coverage_error_z', 'cond_coverage_error_repr'] and type != 'mark':
            #     #axis.set_ylim(bottom=0)
            #     axis.set_yscale('log')
...
def plot_coverage(df, metric='coverage', ncols=4, type='joint'):
    ...
    if size < ncols:
        ncols = size
    fig, ax = plt.subplots(nrows, ncols, figsize=(ncols * 2.1, nrows * 2.4), 
            squeeze=False, sharex=True, sharey=True, dpi=200)
    ax = ax.flatten()
    ...
    for axis, (group_values, df_model) in zip(ax, df_grouped):
        g = sns.lineplot(data=df_model, x='coverage_level', y=metric, hue='conformalizer', hue_order=conformalizer_order,
                         ax=axis, err_style='bars', errorbar=('se', 1), err_kws={'capsize': 3}, marker='o', markeredgewidth=0)
        g.legend().remove()
...
def plot_metrics_on_synthetic_datasets(df, alpha, type):
    ...
    for i, dataset in enumerate(datasets):
        for j, metric in enumerate(metrics):
            ...
            g = sns.lineplot(data=df_plot, x='cal_size', y=metric, hue='conformalizer', hue_order=get_conformalizers_order_joint(), ax=axis, err_style='bars', 
                             errorbar=('se', 1), err_kws={'capsize': 3}, marker='o', markeredgewidth=0)
            g.legend().remove()
            axis.set_xscale('log')
            axis.set_xticks(list(df_plot['cal_size'].unique()), list(df_plot['cal_size'].unique()), rotation=40)
...
def plot_metrics_for_varying_trainingsize(df, alpha, type):
    ...
    if type == 'mark':
        ...
        metrics = ['coverage', 'wsc', 'relative_length', 'gmean_length', 'cond_coverage_error_z']
    metrics = ['coverage']
    conformalizers = df.conformalizer.unique()
    nrows, ncols = len(datasets), len(metrics)
    fig, axes = plt.subplots(nrows, ncols, figsize=(ncols * 2.8, nrows * 2.5), squeeze=False)
    for i, dataset in enumerate(datasets):
        ...
        for j, metric in enumerate(metrics):
            ...
            if type == 'time':
                ...
                order = get_conformalizers_order_joint() 
            g = sns.lineplot(data=df_plot, x='train_size', y=metric, hue='conformalizer', hue_order=order, ax=axis, err_style='bars', 
                                errorbar=('se', 1), err_kws={'capsize': 3}, marker='o', markeredgewidth=0)
            g.legend().remove()
            #axis.set_xscale('log')
            ...
            axis.set_ylabel(f'{map_metrics(metric)}')
