...
from .dist_util import get_logz_samples, icdf
...
class ConformalizerBase:
    def __init__(self, dl_calib, model, alpha, args):
        ...
        # with torch.no_grad():
        self.scores = [self.get_score_on_batch(batch) for batch in dl_calib]
    ...
    def conformal_quantile(self, scores):
        n = scores.shape[0]
        scores = torch.cat([scores, torch.tensor([torch.inf], device=scores.device)])
        level = torch.tensor((1 - self.alpha) * (n + 1)) / (n + 1)
        level = level.type(scores.dtype).to(scores.device)
        return torch.quantile(scores, level, interpolation='higher')
...
class TimeConformalizer(ConformalizerBase):
    ...
    def get_joint_prediction_region(self, past_events):
        ...
        preds = [{label: region for label in range(nb_labels)} for region in time_prediction_regions]
...
class ConformalizerRightBound(TimeConformalizer):
    ...
    def get_time_prediction_region(self, past_events):
        ...
        preds = [[[left.item(), right.item()]] for left, right in zip(last_observed_times, right_bounds)]
        return np.array(preds)
...
class CQR(TimeConformalizer):
    def get_unconformalized_bounds(self, past_events):
        ...
        quantile_low = icdf(self.model, level_low, past_events)[:, 0]
        quantile_up = icdf(self.model, level_up, past_events)[:, 0]
    ...
    def get_time_prediction_region(self, past_events):
        ...
        right_bounds = torch.maximum(last_observed_times, right_bounds)
        preds = [[[left.item(), right.item()]] for left, right in zip(left_bounds, right_bounds)]
        return np.array(preds)
...
class CHDRTime(TimeConformalizer):
    def sanity_check_cdf(self, past_events):
        device = self.args.device
        batch_shape = past_events.times.shape[:1]

        time_sample_at_infinity = torch.tensor([[1000.0]], device=device).expand(batch_shape + (1,))
    ...
    def create_interval(self, seq):
        ...
        seq = torch.cat(
            [
                torch.tensor([False], device=seq.device),
                seq,
                torch.tensor([False], device=seq.device),
            ]
        ).int()
    ...
    def get_time_range(self, method, past_events, nb_steps):
        ...
        if method == 'linear':
            ...
            delta_range = torch.linspace(0 + epsilon, 0.0004, nb_steps, device=device)[None, :].expand(
            ...
            elif method == 'beta_sampling': # We sample more in the tails
        ...
        # Note: on mooc, there is not enough precision to distinguish successive time values
        # For example, time_range[0].detach().cpu().numpy() can contain duplicate values
    ...
    def get_logz_samples(self, past_events, nb_samples=500):
        ...
        # Sample label
        log_density, log_mark_density, y_pred_mask = self.model.log_density(
            query=time_sample, events=past_events
        )
        # Sample Z
    ...
    def get_score(self, past_events, target_time, target_label):
        ...
        log_density, log_mark_density, y_pred_mask = self.model.log_density(
            query=target_time.unsqueeze(1), events=past_events
        )
    ...
    def get_time_prediction_region(self, past_events):
        ...
        # Density evaluation
        log_density, log_mark_density, y_pred_mask = self.model.log_density(
            query=time_range, events=past_events
        )
        log_ground_density = torch.logsumexp(log_density, dim=-1)
...
class MarkPredictor(ConformalizerBase):
    def __init__(self, *args, no_empty_sets=True, **kwargs):
        super().__init__(*args, **kwargs)
        self.no_empty_sets = no_empty_sets
    ...
    def get_time_sample(self, past_events, nb_samples=100):
        ...
        batch_shape = past_events.times.shape[:1]
        epsilon = 1e-4
        alpha = torch.linspace(0 + epsilon, 1 - epsilon, nb_samples, device=device)[None, :].expand(
            batch_shape + (nb_samples,)
        )
...
class APS(MarkPredictor):
    ...
    def get_mark_prediction_region(self, past_events):
        ...
        if self.no_empty_sets:   # Avoid 0 size sets
            ind[:, 0] = True
...
class JointIndependent(ConformalizerBase):
    ...
    def get_joint_prediction_region(self, past_events):
        ...
        for time_pred, label_pred in zip(time_preds, label_preds):
            pred = {label: time_pred for label, in_region in enumerate(label_pred) if in_region}
            if len(pred) == 0:
                print('Warning: Zero-length set encountered')
            preds.append(pred)
        return np.array(preds)
