...
class SepSAHP(MCDecoder):
    ...
    def __init__(
        ...
        super(SepSAHP, self).__init__(
            ...
            marks=marks,
            **kwargs)
        self.mu = nn.Linear(in_features=units_mlp[0], out_features=1, bias=False)
        self.eta = nn.Linear(in_features=units_mlp[0], out_features=1, bias=False)
        self.gamma = nn.Linear(in_features=units_mlp[0], out_features=1, bias=False)
        self.activation = nn.GELU()
        self.activation_gamma = ParametricSoftplus(units=1)
        self.final_activation = ParametricSoftplus(units=1)
    ...
    def log_intensity(
        ...
        mu = self.activation(self.mu(history_representations_time)) #[B,T,K]
