...
class SepLogNormalMixtureDecoder(VariableHistoryDecoder):
    ...
    def __init__(
        ...
        super(SepLogNormalMixtureDecoder, self).__init__(
            name="sep-log-normal-mixture",
    ...
    def forward(
        ...
        if self.multi_labels:
            p_m = th.sigmoid(
                self.marks2(th.tanh(self.marks1(history_representations_mark))))
        else:
            p_m = th.softmax(
                self.marks2(
                    th.tanh(self.marks1(history_representations_mark))), dim=-1)
        p_m = p_m + epsilon(dtype=p_m.dtype, device=p_m.device)
        check_tensor(p_m, positive=True)
        cum_f = w * 0.5 * (
                1 + th.erf((delta_t - mu) / (std * math.sqrt(2))))
        cum_f = th.clamp(th.sum(cum_f, dim=-1), max=1-1e-5)
        #print('cum_f', cum_f[0,0].item(), cum_f[0,1].item()) 
        #print('modif taken into account, bla')
        ...
        marked_log_intensity = base_log_intensity.unsqueeze(
            dim=-1)  # [B,T,1]
        if th.isnan(marked_log_intensity * intensity_mask.unsqueeze(-1)).sum() != 0:
            print('check1')
